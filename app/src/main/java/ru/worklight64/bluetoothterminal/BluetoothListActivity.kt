...
import ru.worklight64.bluetoothterminal.databinding.ActivityBluetoothListBinding
import ru.worklight64.bluetoothterminal.dataclass.BluetoothItem

class BluetoothListActivity : AppCompatActivity(), BluetoothListAdapter.Listener {
    private lateinit var form: ActivityBluetoothListBinding
    private var btAdapter: BluetoothAdapter? = null
    private lateinit var adapter: BluetoothListAdapter

    override fun onCreate(savedInstanceState: Bundle?) {
        form = ActivityBluetoothListBinding.inflate(layoutInflater)
        super.onCreate(savedInstanceState)
        setContentView(form.root)
        adapter = BluetoothListAdapter(this)
    ...
    override fun onClick(item: BluetoothItem) {
        val i = Intent().apply {
